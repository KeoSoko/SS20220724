...
- **Receipt Management**: Includes smart filters (date range, amount range, vendor), multi-receipt continuous scanning mode, and batch gallery import with visual queues and progress tracking. Email-to-receipt supports both file attachments (images/PDFs) and email body extraction using a **Document-First Pipeline**: 1) Store raw email document in `email_documents` table for auditability, 2) Detect vendor via pattern-based matching on sender domain + subject + body phrases (server/vendor-detection.ts), 3) **HTML DOM extraction** using cheerio for supported vendors (server/vendor-extraction-engine.ts) — parses raw HTML table cells and DOM nodes to find totals, dates, order IDs with confidence scoring (total=0.7, date=+0.1, storeName=+0.1, orderId=+0.05), 4) Text-regex fallback extraction for known vendors (server/deterministic-extraction.ts), 5) Fall back to GPT-4.1 AI extraction for unknown vendors, 6) Mark as "needs_review" if AI confidence < 0.5 instead of auto-rejecting. Supported HTML extraction vendors: Pick n Pay, Takealot, Amazon, Checkers, Uber Eats — each with vendor-specific config (totalLabels, dateLabels, orderIdLabels, currencySymbols, optional customExtractor). Extraction methods tracked as `deterministic_html:VendorName` or `deterministic_text:VendorName`. Adding new vendors requires only a new config object in VENDOR_CONFIGS. Inline/embedded images with content-id are always filtered out regardless of size. Email body extraction includes forwarded-content stripping (isolates actual receipt content from reply chains/forwarded headers), signature stripping restricted to bottom 30% with 80% over-stripping fail-safe, and receipt validation (rejects zero-amount, missing store name). Email body storage (html_body, text_body) with reprocess functionality via Command Center UI. **Email HTML Viewer Security**: Sanitization strips all `<script>`, `<link>`, `<meta http-equiv>`, `<base>`, `<form>`, `<iframe>`, `<object>`, `<embed>` tags; removes inline event handlers and javascript: URIs; replaces external `<img src="https://...">` with placeholders to prevent tracking pixel leaks; strips CSS `url(https://...)` background references; CSP header blocks all images and fonts (`img-src 'none'`); iframe uses `sandbox=""` (most restrictive) with `srcDoc` rendering via authenticated fetch.
...
- **Custom Category Management**: Allows users to create and filter receipts by custom categories. Category renames cascade atomically to all associated receipt labels within the workspace via a DB transaction, keeping filters, exports, and reports consistent.
- **Merchant Category Learning**: Workspace-scoped learning table (`merchant_category_rules`) stores user corrections — when a user assigns a custom category label to a receipt, the system saves a rule for that merchant. Future receipts from the same merchant (case-insensitive match) automatically receive the same label without AI intervention. Rules update when the user corrects a merchant again. Safety guard: store names must be > 2 characters. Logged as `[merchant-learning]` in server output. Storage methods: `getMerchantCategoryRule`, `upsertMerchantCategoryRule`.
- **Category Engine**: Unified frontend utility (`client/src/utils/category-engine.ts`) provides `resolveCategory`, `formatCategoryLabel`, and `normalizeCategory` used consistently across receipt cards, filter dropdowns, and comparisons. Raw enum slugs (e.g. `dining_takeaways`) are always displayed as human-readable labels (e.g. `Dining Takeaways`).
